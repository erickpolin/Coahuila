...
attach(ConcCoahuila) #this is for not writing the name of the data frame and the $ ever time

#Sort Conc according to INGCOR, FOLIOVIV, FOLIOHOG
ConcCoahuila<- orderBy (~+ing_cor+folioviv+foliohog, data=ConcCoahuila) #this give us the households sorted by total income

#Adding the values of the expansion FACTOR. The sum is 34 million, which is the number of households in the country.
tot_hogares<-sum(ConcCoahuila$factor,to.data.frame=TRUE)
...
ConcCoahuila$tam_dec<-tam_dec

# Creating Deciles of Income 

ConcCoahuila$MAXT<-ConcCoahuila$ing_cor #vamos a crear en esta base la variable MAXT que es una copia de la columna de ingresos

ConcCoahuila<-ConcCoahuila[with(ConcCoahuila, order(rank(MAXT))),]  #lo que hicimos aqu? fue reordenar la base con respecto a MAXT. Cosa que en realidad, ya estaba.

ConcCoahuila$ACUMULA<-cumsum(ConcCoahuila$factor) #aqu? creamos una variable de suma acumulada del FACTOR de viviendas.
...
for(i in 1:9)
{
  a1<-ConcCoahuila[dim(ConcCoahuila[ConcCoahuila$ACUMULA<tam_dec*i,])[1]+1,]$factor
  ConcCoahuila<-rbind(ConcCoahuila[1:(dim(ConcCoahuila[ConcCoahuila$ACUMULA<tam_dec*i,])[1]+1),],
                             ConcCoahuila[(dim(ConcCoahuila[ConcCoahuila$ACUMULA<tam_dec*i,])[1]+1):dim(ConcCoahuila[1])[1],])
  
  b1<-tam_dec*i-ConcCoahuila[dim(ConcCoahuila[ConcCoahuila$ACUMULA<tam_dec*i,])[1],]$ACUMULA
  ConcCoahuila[(dim(ConcCoahuila[ConcCoahuila$ACUMULA<tam_dec*i,])[1]+1),]$factor<-b1
  ConcCoahuila[(dim(ConcCoahuila[ConcCoahuila$ACUMULA<tam_dec*i,])[1]+2),]$factor<-(a1-b1)
}

#aqu? estamos creando otra variable de suma acumulada del n?mero de hogares
ConcCoahuila$ACUMULA2<-cumsum(ConcCoahuila$factor)

#aqu? estamos creando una variable que se llama decil que solo tiene ceros
ConcCoahuila$DECIL<-0
...
ConcCoahuila[(ConcCoahuila$ACUMULA2<=tam_dec),]$DECIL<-1
...
for(i in 1:9)
{
  ConcCoahuila[((ConcCoahuila$ACUMULA2>tam_dec*i)&(ConcCoahuila$ACUMULA2<=tam_dec*(i+1))),]$DECIL<-(i+1)
}

# a lo que le qued? cero (que es la ?ltima observaci?n), ponle el decil 10
ConcCoahuila[ConcCoahuila$DECIL%in%"0",]$DECIL<-10

setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Coahuila")
write.dbf(ConcCoahuila,file="ConcCoahuila2010.dbf")
...
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Coahuila")
write.dbf(ConcCoahuila,file="ConcCoahuila2018.dbf")
...
#reading the data
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Coahuila")
Conc2010<-read.dbf("ConcCoahuila2010.dbf",as.is = T)
...
write.dbf(c_DECIL_ES,file = "Coahuila por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Coahuila por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Coahuila por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Coahuila por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Coahuila por fuente por DECIL LS 2010.dbf")
...
write.dbf(c_DECIL_ES,file = "Coahuila INDIGENA por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Coahuila INDIGENA por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Coahuila INDIGENA por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Coahuila INDIGENA por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Coahuila INDIGENA por fuente por DECIL LS 2010.dbf")
...
write.dbf(c_DECIL_ES,file = "Coahuila NO por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Coahuila NO por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Coahuila NO por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Coahuila NO por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Coahuila NO por fuente por DECIL LS 2010.dbf")
...
#reading the data
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Coahuila")
Conc2018<-read.dbf("ConcCoahuila2018.dbf",as.is = T)
...
write.dbf(c_DECIL_ES,file = "Coahuila Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Coahuila Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Coahuila Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Coahuila Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Coahuila Ingresos por fuente por DECIL LS 2018.dbf")
...
write.dbf(c_DECIL_ES,file = "Coahuila INDIGENA Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Coahuila INDIGENA Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Coahuila INDIGENA Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Coahuila INDIGENA Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Coahuila INDIGENA Ingresos por fuente por DECIL LS 2018.dbf")
...
write.dbf(c_DECIL_ES,file = "Coahuila NO Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Coahuila NO Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Coahuila NO Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Coahuila NO Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Coahuila NO Ingresos por fuente por DECIL LS 2018.dbf")
...
Deciles2010<-read.dbf("Coahuila por fuente por DECIL estimaciones 2010.dbf")
names(Deciles2010)[1]<-c("ingcor2010")

Deciles2018<-read.dbf("Coahuila Ingresos por fuente por DECIL estimaciones 2018.dbf")
...
deciles2010No<-read.dbf("Coahuila NO por fuente por DECIL estimaciones 2010.dbf")
names(deciles2010No)[1]<-c("No2010")

deciles2018No<-read.dbf("Coahuila NO Ingresos por fuente por DECIL estimaciones 2018.dbf")
...
deciles2010indigena<-read.dbf("Coahuila INDIGENA por fuente por DECIL estimaciones 2010.dbf")
names(deciles2010indigena)[1]<-c("Indigena2010")

deciles2018indigena<-read.dbf("Coahuila INDIGENA Ingresos por fuente por DECIL estimaciones 2018.dbf")
